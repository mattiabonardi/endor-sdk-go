...
func (h *AbstractResourceService) create(c *EndorContext[CreateDTO[ResourceInstance[*DynamicResource]]]) (*Response[ResourceInstance[*DynamicResource]], error) {
	...
	return NewResponseBuilder[ResourceInstance[*DynamicResource]]().AddData(created).AddSchema(h.rootSchema).AddMessage(NewMessage(Info, fmt.Sprintf("%s created", h.resource))).Build(), nil
}
