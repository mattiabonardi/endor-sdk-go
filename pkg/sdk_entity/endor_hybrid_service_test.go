...
func TestEndorHybridService(t *testing.T) {
	...
	// check attribute
	assert.Equal(t, endorService.Entity, "hybrid-service")
	assert.Equal(t, endorService.EntityDescription, "Hybrid Service (EndorHybridService)")
	// check schema
	assert.Equal(t, len(*endorService.EntitySchema.Properties), 3)
