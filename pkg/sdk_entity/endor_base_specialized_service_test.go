...
func TestEndorBaseSpecializedService(t *testing.T) {
	...
	// check attribute
	assert.Equal(t, endorService.Entity, "base-specialized-service")
	assert.Equal(t, endorService.EntityDescription, "Base Specialized Service (EndorBaseSpecializedService)")
	// check schema
	assert.Equal(t, 3, len(*endorService.EntitySchema.Properties))
